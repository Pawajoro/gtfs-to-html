...
function formatStopTime(stoptime, timetable, config) {
  ...
  if (stoptime.timepoint === undefined && stoptime.departure_time === undefined && !stoptime.noStop) {
    stoptime.noService = true;
    stoptime.classes.push('noService');
  ...
  if (stoptime.noStop) {
    stoptime.classes.push('noStop');
    if (timetable.noStopSymbol !== null) {
      timetable.noStopSymbolUsed = true;
    }
  }
...
exports.formatDaysLong = (dayList, config) => {
  const mapObject = _.zipObject(config.daysShortStrings, config.daysStrings);
...
exports.formatStops = (stops, timetable, config) => {
  for (const trip of timetable.orderedTrips) {
    ...
    for (const [idx, stoptime] of trip.stoptimes.entries()) {
	  ...
	  // Edited by Pawajoro - get first and last stop id of that trip
	  if (idx === 0) {
    ...
    for (const stop of stops) {
	  if (stop.stop_id === lastStop && onTrip) {
		onTrip = false;
	  }

      const lastStopTime = _.last(stop.trips);
      if ((!lastStopTime || lastStopTime.trip_id !== trip.trip_id) && !onTrip) {
        stop.trips.push(formatStopTime({}, timetable, config));
      }
	  if ((!lastStopTime || lastStopTime.trip_id !== trip.trip_id) && onTrip) {
        stop.trips.push(formatStopTime({noStop: true}, timetable, config));
      }

	  if (stop.stop_id === firstStop && !onTrip) {
		onTrip = true;
	  }
    }
  }
...
exports.formatTimetableLabel = timetable => {
  ...
  if (timetable.routes[0].route_short_name !== '' && timetable.routes[0].route_short_name !== undefined) {
    timetableLabel += timetable.routes[0].route_short_name;
  } else if (timetable.routes[0].route_long_name !== '' && timetable.routes[0].route_long_name !== undefined) {
  ...
  if (timetable.stops && timetable.stops.length > 0) {
    ...
    if (firstStop === lastStop) {
      if (timetable.routes[0].route_long_name !== '' && timetable.routes[0].route_long_name !== undefined) {
        timetableLabel += ` - ${timetable.routes[0].route_long_name}`;
      }
