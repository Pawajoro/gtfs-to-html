  ...
  if (_.includes(['beginning','end'], config.sortingAlgorithm)) {
    ...
    for (const stop of referenceStoptimes) {
      ...
      for (const trip of sortedTrips) {
        ...
        let selectedStoptime;
        selectedStoptime = _.find(trip.stoptimes, {stop_id: stop.stop_id});
 ...
 * is present.
 */
const convertRouteToTimetablePage = (route, direction, calendars, config) => {
  const timetable = {
  ...
  const timetablePages = await Promise.all(routes.map(async route => {
    ...
    return Promise.all(_.map(directionGroups, directionGroup => {
      ...
      return Promise.all(_.map(dayGroups, calendars => {
        return convertRouteToTimetablePage(route, direction, calendars, config);
      }));
    }));
  }));
...
const getFrequenciesByTrip = async (trip, config) => {
  ...
  return frequencies.map(_.curryRight(formatters.formatFrequency)(config));
};
 ...
 */
const getTimetablePages = async (agencyKey, config) => {
...
exports.getFormattedTimetablePages = async (agencyKey, config) => {
  const timetablePages = await getTimetablePages(agencyKey, config);

  return Promise.all(timetablePages.map(_.curry(async (config, timetablePage) => {
    timetablePage.consolidatedTimetables = await consolidateTimetables(timetablePage.timetables, config);
