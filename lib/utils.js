...
/*
 * Get all of the route text colors for a timetable page.
 */
const getTimetablePageTextColors = async timetablePage => {
  const routes = await gtfs.getRoutes({
    agency_key: timetablePage.agency_key,
    route_id: { $in: timetablePage.routeIds }
  });
  return _.compact(_.uniq(_.map(routes, 'route_text_color')));
};
...
/*
 * Get the routes for a specific timetable.
 */
const getRoutesFromTimetable = async timetable => {
...
const convertTimetableToTimetablePage = async (timetable, config) => {
  if (!timetable.routes) {
    timetable.routes = await getRoutesFromTimetable(timetable);
  }
...
const getTripsFromTimetable = async (timetable, calendars, config) => {
  ...
  if (trips.length === 0) {
    config.logWarning(`No trips found for route_id=${timetable.route_ids.join('_')}, direction_id=${timetable.direction_id}, service_ids=${JSON.stringify(timetable.serviceIds)}, timetable_id=${timetable.timetable_id}`);
  }
  ...
  await Promise.all(trips.map(async trip => {
    ...
    if (formattedTrip.stoptimes.length === 0) {
      config.logWarning(`No stoptimes found for agency_key=${timetable.agency_key}, trip_id=${formattedTrip.trip_id}, route_id=${timetable.route_id.join('_')}, timetable_id=${timetable.timetable_id}`);
    }
...
const formatTimetables = async (timetables, config) => {
  return Promise.all(timetables.map(async timetable => {
    ...
    Object.assign(timetable, {
      ...
      noStopSymbol: config.noStopSymbol,
...
const mergeTimetablesWithSameId = timetables => {
  ...
  const mergedTimetables = _.groupBy(timetables, 'timetable_id');

  return Object.values(mergedTimetables).map(timetableGroup => {
    const mergedTimetable = _.omit(timetableGroup[0], 'route_id');
 ...
 */
const getTimetablePageById = async (agencyKey, timetablePageId, config) => {
  // Check if there are any timetable pages defined in timetable_pages.txt
  ...
  if (timetables.length > 0 && timetablePages.length > 0) {
  ...
  return convertRouteToTimetablePage(routes[0], direction, calendars, calendarDates, config);
};
...
exports.getFormattedTimetablePage = async (agencyKey, timetablePageId, config) => {
  const timetablePage = await getTimetablePageById(agencyKey, timetablePageId, config);
  ...
  timetablePage.routeTextColors = await getTimetablePageTextColors(timetablePage);
